...
if __name__ == "__main__":
    ...
    fetch_logfile = logging.FileHandler("./logfile/EdimaxPM2.5_fetch_" + datetime.now().strftime("%Y-%m-%d") + ".log", "a")
    ...
    if (r.status_code/100) == 2:
        ...
        #Strange condition occur where status is 'ok' but data is NoneType
        if (response_obj["devices"] is None):
            logs.warning("Strange data received: " + r.text);
        else:
            logs.info("Number of data received: " + str(len(response_obj["devices"])))
            for x in response_obj["devices"]:
            myCursor.execute( "INSERT IGNORE INTO `Edimax_devices_info` (`id`, `name`, `lat`, `lon`, `org`, `area`, `type`) VALUES (\"" + x["id"] + "\",\"" + x["name"] + "\",\'" + str(x["lat"]) + "\',\'" + str(x["lon"]) + "\',\"" + x["org"] + "\",\'" + str(x["area"]) + "\',\"" + x["type"] + "\");")
            myCursor.execute( "INSERT IGNORE INTO `Edimax_devices_data` (`id`, `pm25`, `pm10`, `co2`, `hcho`, `tvoc`, `co`, `t`, `h`, `time`) VALUES (\"" + x["id"] + "\",\'" + str(x["pm25"]) + "\',\'" + str(x["pm10"]) + "\',\'" + str(x["co2"]) + "\',\'" + str(x["hcho"]) + "\',\'" + str(x["tvoc"]) + "\',\'" + str(x["co"]) + "\',\'" + str(x["t"]) + "\',\'" + str(x["h"]) + "\',\"" + x["time"] + "\");")
            myConnection.commit()
        ...
        #Wait for 5 minutes to retry data retrieval
        time.sleep (300)
